  ...
  exports.cssCompilers = cssCompilers = {
    styl: {
      ...
      compileSync: function(sourcePath, source) {
        ...
        libs.stylus(source, options).use(libs.bootstrap()).use(libs.nib()).use(libs.bootstrap()).set('compress', this.compress).set('include css', true).define('url', libs.stylus.url()).render(callback);
        return result;
      }
    }
  };
